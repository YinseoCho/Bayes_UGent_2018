...
This is a hands-on introduction to the ```tidyverse```, a collection of **R** pacakages for data manipulation and plotting. There is no time to cover all the amazing things you can do with these packages. If you want to learn more, I suggest [R for Data Science](http://r4ds.had.co.nz/) by [Garrett Grolemund](https://www.oreilly.com/pub/au/5570) and [Hadley Wickham](http://hadley.nz/).
...
The symbol **```%>%```** is the **pipe operator** , which allows to serially concatenate functions to the same data frame. Read it as *"and then"*. In the example below, we called the data frame ```att``` **and then** applied the ```filter``` function to discard participant #99.

## Variable selection

For the exercises in this course, we will only need a subset of variables in this dataset.   
   
Researchers were interested in reducing binge drinking, so it makes sense to focus our attention on **negative** imagery and one alcoholic beverage. We choose **beer**, because we are in Belgium. We also need control conditions, i.e., **neutral** imagery and **water**.
...
* **long** format: one row per **observation**, **columns** identify **conditions**
...
It is often required to provide summary statistics of the data. How to do it with the ```tidyverse```?
...
read.table(
    ...
    rename(participant = ssj,
           ...
           water_negative = waterneg,
           water_neutral = waterneu) %>%
    filter(water_neutral >= -10) %>%
    gather(key = "condition",
           value = "ratings",
           wine_negative:water_neutral) %>%
    separate(condition, c("drink", "imagery")) %>%
    mutate(drink = factor(drink),
           imagery = factor(imagery)) %>%
    write_csv(.,
              paste0(getwd(), "/data_attitude_exercise.csv")) %>%
    group_by(drink, imagery, gender) %>%
    summarize(n = n(),
...
read_csv(paste0(getwd(), "/data_attitude_exercise.csv")) %>%
    group_by(drink, imagery, gender) %>% 
    summarize(mean = mean(ratings),
              ci.95 = 1.96 * sd(ratings)/sqrt(n())) %>%
    ggplot(.,
           aes(x = drink,
               y = mean,
               fill = imagery)) +
...
read_csv(paste0(getwd(), "/data_attitude_exercise.csv")) %>%
    mutate(imagery = recode(imagery,
                            "negative" = "neg",
                            "neutral" = "neut")) %>%
    pirateplot(formula = ratings ~ imagery + drink + gender,
               data = .,
               main = "mean ratings",
               ylim = c(-25, 25),
               ylab = "",
...
The aim of this study was to assess whether negative imagery would influence the likeness ratings of alcoholic beverages. It's time to statistically verify the hypothesis.   
...
It is good practice to report **effect sizes** along with *p*-values, so that readers can make their own mind with respect to the importance of the observed effects. Even better, you could report **confidence intervals** around effect sizes, so that readers can have a clearer picture of the *precision* of your estimation.   
   
I particularly like the idea of **bootstrapping** effect sizes, a better approach when the data are known not to be normally distributed or when the distribution is unknown[^2]. I will show you how to do it using the package ```bootES```[^3].   
...
```{r exercise_3_1_rmANOVA, message = FALSE}
# ANOVA
ex31.rmANOVA.att <- 
    read_csv(paste0(getwd(), "/data_attitude_exercise.csv")) %>%
    aov_ez("participant",
...
```{r exercise_3_1_paired_contrasts, message = FALSE, warning = FALSE}
# paired contrasts
ex31.posthoc.att <-
    emmeans(ex31.rmANOVA.att, ~ imagery:drink) %>%
    pairs(.,
          test = adjusted("free")) %>%
    as.glht(.) %>%
    summary(.)
...
ex31.att.bootES <-
    read_csv(paste0(getwd(), "/data_attitude_exercise.csv")) %>%
    unite("condition", c(drink, imagery)) %>%
    spread(condition, ratings) %>%
    mutate(wine_negativeVSwine_neutral = wine_negative - wine_neutral,
           ...
           water_negativeVSwater_neutral = water_negative - water_neutral) %>%
    dplyr::select(-c(water_negative:wine_neutral)) %>%
    gather(key = "diff.conds",
           value = "ratings",
           wine_negativeVSwine_neutral:water_negativeVSwater_neutral)
...
# summary table
summary.ex31.att.bootES <- data.frame(magrittr::extract2(ex31.posthoc.att, 1)$object,
                                 "Pearson.r" = NA,
                                 "CI.95.low" = NA,
                                 "CI.95.high" = NA,
                                 "bias" = NA,
...
# OH MY GOD ANOTHER LOOP IN R!
for(i in 1:length(unique(ex31.att.bootES$diff.conds))) {
    summary.ex31.att.bootES[i, 7:11] <- summary(magrittr::extract2(ex31.att.PearsonR, i))
}

summary.ex31.att.bootES
```
...
```{r exercise_3_2_mixedANOVA, message = FALSE}
# ANOVA
ex32.rmANOVA.att <- 
    read_csv(paste0(getwd(), "/data_attitude_exercise.csv")) %>%
    aov_ez("participant",
...
```{r exercise_3_2_paired_contrasts, message = FALSE}
# paired contrasts (only a subset of them)
ex32.posthoc.att <-
    emmeans(ex32.rmANOVA.att, ~ gender|imagery|drink) %>%
    pairs(.,
          test = adjusted("free")) %>%
    as.glht(.) %>%
    summary(.)
...
# summary table
summary.ex32.att.bootES <- data.frame("contrast" = c("negative, wine, female - male",
                                                     "neutral, wine, female - male",
                                                     "negative, water, female - male",
                                                     "neutral, water, female - male"),
                                      "estimate" = NA,
                                      "SE" = NA,
                                      "df" = NA,
                                      "t.ratio" = NA,
...
# OH MY GOD ANOTHER LOOP IN R! THIS IS GETTING RIDICULOUS!
for(i in 1:length(unique(ex32.att.bootES$condition))) {
    # subsetting this list turned out to be more complicated than I thought... here is a very inelegant workaround
    summary.ex32.att.bootES[i, "estimate"] <- magrittr::extract2(ex32.posthoc.att, i)$test$coefficients
    summary.ex32.att.bootES[i, "SE"] <- magrittr::extract2(ex32.posthoc.att, i)$test$sigma
    summary.ex32.att.bootES[i, "df"] <- magrittr::extract2(ex32.posthoc.att, i)$df
    summary.ex32.att.bootES[i, "t.ratio"] <- magrittr::extract2(ex32.posthoc.att, i)$test$tstat
    summary.ex32.att.bootES[i, "p.value"] <- magrittr::extract2(ex32.posthoc.att, i)$test$pvalues
    summary.ex32.att.bootES[i, 7:11] <- summary(magrittr::extract2(ex32.att.CohenD, i))
}

summary.ex32.att.bootES
```

That's it. I hope I managed to at least give you a glimpse of the versatility, readability, and user-friendliness of the ```tidyverse```.
...
[^1]: This is a modified version of a dataset included in the book [Discovering Statistics Using R (Field, Miles & Field, 2012)](https://www.discoveringstatistics.com/books/discovering-statistics-using-r/).

[^2]: If you prefer **not** to bootstrap your effect sizes, you can use the ```MBESS``` package (a useful tutorial can be found [here](http://osc.centerforopenscience.org/2014/03/06/confidence%20intervals/)). If you want to do it in **SPSS**, see [here](https://daniellakens.blogspot.be/2014/06/calculating-confidence-intervals-for.html).

[^3]: A clear explanation of how to use ```bootES``` is provided by the authors of the package in their [paper](https://link.springer.com/article/10.3758/s13428-013-0330-5).
